    ...
    script {
        ...
        try {
            sh '''
                ...
                ssh -o 'StrictHostKeyChecking no' -i ${WORKSPACE}/deploy/id_rsa root@${OCP_IP} "chmod +x logging_vars.sh; source logging_vars.sh; echo "${CHANNEL}""
                ssh -o 'StrictHostKeyChecking no' -i ${WORKSPACE}/deploy/id_rsa root@${OCP_IP} "cd /root/ocp_ansible_validation/; BASTION_HOSTNAME=`hostname -s`; echo $BASTION_HOSTNAME; sed -i 's|BASTION_HOSTNAME|'"${BASTION_HOSTNAME}"'|g' cl_inventory" 
